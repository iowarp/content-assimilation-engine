...
#ifndef CAE_ADAPTER_METADATA_MANAGER_H
#define CAE_ADAPTER_METADATA_MANAGER_H
...
public:
  ...
  bool Create(const File &f, std::shared_ptr<AdapterStat> &stat) {
    ...
    hshm::ScopedRwWriteLock md_lock(lock_, hermes::kMDM_Create);
  ...
  bool Update(const File &f, const AdapterStat &stat) {
    ...
    hshm::ScopedRwWriteLock md_lock(lock_, hermes::kMDM_Update);
  ...
  bool Delete(const std::string &path, const File &f) {
    ...
    hshm::ScopedRwWriteLock md_lock(lock_, hermes::kMDM_Delete);
    auto iter = hermes_file_to_stat_.find(f);
    if (iter != hermes_file_to_stat_.end()) {
  ...
  std::list<File> *Find(const std::string &path) {
    ...
    hshm::ScopedRwReadLock md_lock(lock_, hermes::kMDM_Find);
  ...
  std::shared_ptr<AdapterStat> Find(const File &f) {
    hshm::ScopedRwReadLock md_lock(lock_, hermes::kMDM_Find2);
    auto iter = hermes_file_to_stat_.find(f);
    if (iter == hermes_file_to_stat_.end())
      return nullptr;
    else
...
#define CAE_FS_METADATA_MANAGER                                                \
  hshm::Singleton<::cae ::MetadataManager>::GetInstance()
#define CAE_FS_METADATA_MANAGER_T cae ::MetadataManager *

#endif // CAE_ADAPTER_METADATA_MANAGER_H
